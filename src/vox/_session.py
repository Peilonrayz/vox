...
class Session:
    ...
    def __init__(self, linter: vox.linters.BaseLinter, session, messages, notify) -> None:
        self._linter = linter
    ...
    def nox_run(self, *args, silent=True, success_codes=None, **kwargs):
        ...
        ret = self._session.run(*args, **kwargs, silent=silent, success_codes=_success_codes)
    ...
    def run(self, command: Optional[flaggy.Flags] = None):
        ...
        if not isinstance(output, str):
            return code, iter([])
        return code, self._linter.extract_errors(
            {linty.helpers.Task.NAME: self._linter.NAME},
            output,
        )
